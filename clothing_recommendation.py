...
def generateLLMResponse(weather, location, clothes, events):
    ...
    content = f"Tomorrow I will be in {location}. Here is the weather tomorrow in {location} : {weather}. Consider the weather throughout the entire day. Prioritise the warmest temperatures and the conditions during normal daytime hours. Do not recommend a jacket solely because brief rain appears in the forecast if temperatures remain above 22°C.. Here is a list of the clothes that I own : {clothes}. Here are the events that I have today {events}. Based on this, give me 2 outfit recommendations using the clothes that I have - 1 outfit that would suit the weather conditions and the events I have tomorrow perfectly, and 1 backup option. If necessary, I can pack a bag with another set of clothes if an event I have requires a change of clothes, but don't include multiple outfit changes - I can only take 1 bag with 1 outfit inside! The colour of the clothing should be considered when making a decision - both the top and bottom ideally shouldn't be the same unless required. Give me a confidence score out of 10 for both of these options. Don't add extra details about the clothes, only use the details that are provided. Return your response in a JSON format like this: {jsonFormat}. Return only the JSON object. Don't include markdown or any explanations!"
    ...
    for i in range(MAX_ATTEMPTS):
      try:
        ...
        if i == MAX_ATTEMPTS-1:
           raise google.genai.errors.ServerError
        else:
            waitTime = 15 * (2 ** i)

        print(
            f"Gemini server busy. "
            f"Retrying in {waitTime} seconds "
            f"({i + 1}/{MAX_ATTEMPTS})..."
        )
